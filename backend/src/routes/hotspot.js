...
async function issueResetToken(mtId) {
  ...
  const u = users.find(x => x['.id'] === mtId);
  if (!u) return { error: 'user not found', status: 404 };
  const token = crypto.randomBytes(24).toString('hex');
  const expiresAt = new Date(Date.now() + TOKEN_TTL_MS);
  await query(
    `UPDATE hotspot_password_reset_tokens
        SET used_at = NOW()
      WHERE username = $1 AND used_at IS NULL`,
    [u.name]
  );
  await query(
    `INSERT INTO hotspot_password_reset_tokens (token, username, mt_id, expires_at)
     VALUES ($1, $2, $3, $4)`,
...
router.patch('/users/:id', auth, async (req, res) => {
  try {
    const result = await mikrotik.updateHotspotUser(req.params.id, req.body);
    if (req.body?.password) {
      const users = await mikrotik.getHotspotUsers();
      const u = users.find(x => x['.id'] === req.params.id);
      if (u?.name) {
        await mikrotik.kickHotspotByUser(u.name).catch(() => {});
        await logPasswordEvent({ req, service: 'hotspot', action: 'admin_changed_password', target: u.name });
      }
    }
    res.json(result);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
// Admin: generate a single-use reset link for a hotspot user (TTL 10 min)
router.post('/users/:id/reset-token', auth, async (req, res) => {
  try {
    const r = await issueResetToken(req.params.id);
    if (r.error) return res.status(r.status).json({ error: r.error });
    await logPasswordEvent({ req, service: 'hotspot', action: 'reset_link_generated', target: r.user.name });
    res.json({ token: r.token, username: r.user.name, expires_at: r.expiresAt.toISOString(), ttl_ms: TOKEN_TTL_MS });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// Admin: generate reset link and email it to the user (TTL 10 min)
router.post('/users/:id/send-reset-email', auth, async (req, res) => {
  try {
    ...
    const url = `${baseUrl}/reset-hotspot/${r.token}`;
    const minutes = Math.round(TOKEN_TTL_MS / 60000);
    const subject = `รีเซ็ตรหัสผ่าน Hotspot — ${r.user.name}`;
    const text = [
      `สวัสดีครับ/ค่ะ`,
      ``,
      `เราได้รับคำขอรีเซ็ตรหัสผ่าน Hotspot สำหรับ username: ${r.user.name}`,
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;font-size:14px;color:#222;line-height:1.6">
        <p>สวัสดีครับ/ค่ะ</p>
        <p>เราได้รับคำขอรีเซ็ตรหัสผ่าน Hotspot สำหรับ username: <strong>${r.user.name}</strong></p>
    ...
    if (!ok) {
      await logPasswordEvent({ req, service: 'hotspot', action: 'reset_email_sent', target: r.user.name, status: 'failed', details: { email } });
      return res.status(500).json({ error: 'ส่งอีเมลไม่สำเร็จ — กรุณาตรวจสอบการตั้งค่า SMTP' });
    }
    await logPasswordEvent({ req, service: 'hotspot', action: 'reset_email_sent', target: r.user.name, details: { email } });
  ...
  try {
    const r = await query(
      `SELECT username, expires_at, used_at
         FROM hotspot_password_reset_tokens
        WHERE token = $1`,
      [req.params.token]
    );
...
router.post('/reset/:token', async (req, res) => {
  try {
    ...
    await mikrotik.kickHotspotByUser(u.name).catch(() => {});
    await query(
      `UPDATE hotspot_password_reset_tokens SET used_at = NOW() WHERE token = $1`,
      [req.params.token]
    );
    await logPasswordEvent({ req, service: 'hotspot', action: 'user_reset_via_link', target: u.name });
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
