...
export default function VpnReset() {
  ...
  async function submit(values) {
    ...
    setSubmitting(true);
    try {
      await pub.post(`/vpn/reset/${token}`, {
  ...
  if (done) {
    return wrap(<Result status="success" title="เปลี่ยนรหัสผ่านสำเร็จ" subTitle={`Username: ${state.username} — กรุณาใช้รหัสใหม่ในการเชื่อมต่อ VPN ครั้งต่อไป`} />);
  }

  return wrap(
    <>
      <Title level={4} style={{ marginTop: 0 }}>เปลี่ยนรหัส VPN</Title>
